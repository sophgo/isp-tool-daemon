...
int daemon_pipe_cfg_init(const char *json_path, daemon_pipe_cfg_t **p_pipe_cfg)
{
	...
	if (cvi_json_object_object_get_ex(json_obj, "video-src-info", &val_json_object)) {
		...
		for (int i = 0; i < dev_num; ++i) {
			...
			if (cvi_json_object_object_get_ex(array_ele, "enable-teaisp-drc", &arr_val_json_object)) {
				const char *tmp_str = cvi_json_object_get_string(arr_val_json_object);

				if (strcmp(tmp_str, "true") == 0) {
					p_cfg[i].video_pipe_cfg.enable_teaisp_drc = 1;
				} else {
					p_cfg[i].video_pipe_cfg.enable_teaisp_drc = 0;
				}
			}
			...
			if (cvi_json_object_object_get_ex(array_ele, "enable-hdmi", &arr_val_json_object)) {
				...
				if (strcmp(tmp_str, "true") == 0) {
					p_cfg[i].video_pipe_cfg.enable_hdmi = 1;
				} else {
					p_cfg[i].video_pipe_cfg.enable_hdmi = 0;
				}
			}
			...
			if (cvi_json_object_object_get_ex(array_ele, "gop", &arr_val_json_object)) {
				p_cfg[i].video_pipe_cfg.gop = cvi_json_object_get_int(arr_val_json_object);
			}

			if (cvi_json_object_object_get_ex(array_ele, "bitrate", &arr_val_json_object)) {
				p_cfg[i].video_pipe_cfg.bitrate = cvi_json_object_get_int(arr_val_json_object);
			}
...
static int print_daemon_pipe_cfg(daemon_pipe_cfg_t *p_cfg)
{
	for (int i = 0; i < p_cfg[0].dev_num; ++i) {
		...
		printf("\tgop: %d\n", p_cfg[i].video_pipe_cfg.gop);
		printf("\tbitrate: %d\n", p_cfg[i].video_pipe_cfg.bitrate);
